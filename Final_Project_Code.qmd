...
Creating model1, our unadjusted bivariate model. This will show the relationship between these two variables before adjusting for any covariates. This linear regression will be shown in both odds and probabilities for readability's sake.
...
tbl_summary(
  ...
  statistic = all_continuous() ~ c("{mean} ({sd})", "{median} 
                                   ({p25}-{p75})", "{min}-{max}"),
  missing_text = "Missing Data") |>  
add_p(test.args = list(simulate.p.value = TRUE)) |> # Throwing an error and recommending simulated p values despite already doing that...
modify_header(label = "**Variable**") |>
